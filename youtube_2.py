...
cur.execute(sql)
movieList = [i for i in cur.fetchall()]
print(movieList)
...
for m in movieList:
    try:
        movieSeq = str(m[0])
        movieTitle = m[1]
        print(str(movieSeq), movieTitle)
        search = movieTitle + '리뷰'
        r_results = YoutubeSearch(search, max_results=4).to_dict()
        # print(r_results)
        idx = 0
        for i in r_results:
            idx += 1
            r_thumbnails = i['thumbnails'][-1]
            r_url = i['url_suffix']
            url = "https://www.youtube.com/embed/"+r_url.split("v=")[-1]+"?autoplay=0"
            thumbfilename = "uf_" + str(movieSeq) + "_" + str(idx) + ".jpg"
            # print("r_thumbnails")
            # print(r_thumbnails)
            # print("url")
            # print(url)
            # sql = "INSERT INTO YOUTUBE_REVIEW VALUES(REVIEW_SEQ.nextval," + str(movieSeq) + ",'" + r_thumbnails + "','" + r_url + "')"
            sql = "INSERT INTO YOUTUBE_REVIEW VALUES(REVIEW_SEQ.nextval," + str(movieSeq) + ",'" + str(r_thumbnails) + "','" + str(url) + "','" + thumbfilename + "')"
